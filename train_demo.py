from fewshot_re_kit.data_loader import get_loader, get_loader_codalab, get_loader_pair, get_loader_unsupervised, get_loader_semi, load_relation_info_dict, FewRelDatasetSemi
from fewshot_re_kit.framework import FewShotREFramework
from fewshot_re_kit.sentence_encoder import CNNSentenceEncoder, BERTSentenceEncoder, BERTSemiSentenceEncoder, BERTPAIRSentenceEncoder, RobertaSentenceEncoder, RobertaPAIRSentenceEncoder
import models
...
# from models.mrm_test import MRM
from models.mrm2 import MRM
import sys
import torch
...
from utils import _worker_init_fn_
from fewshot_re_kit.data_loader import FewRelDataset as BertDataset
from fewshot_re_kit.data_loader import FewRelDatasetWithPseudoLabel as PBertDataset
...
def main():
    ...
    parser.add_argument('--dropout', default=0.0, type=float,
           help='dropout rate')
    parser.add_argument("--h_alpha", type=float, default=0.4)
    parser.add_argument("--t_alpha", type=float, default=0.8)
    # 查询集中NOTA的比例
    # 按照，原代码15%时nota_rate写1.5，50%时nota_rate写5
    parser.add_argument('--na_rate', default=0, type=float,
           help='NA rate (NA = Q * na_rate)')
    parser.add_argument('--pns_rate', default=0.2, type=float,
            help='pseudo negative sampling rate')
    ...
    parser.add_argument('--semi', action='store_true',
           help='use semi data argument')
    parser.add_argument('--mrm', action='store_true',
           help='use semi data argument')
    # FewRel测试集上的效果
    parser.add_argument('--codalab', action='store_true',
           help='use codalab to test model')
    ...
    if encoder_name == 'cnn':
        ...
        if opt.pair:
            ...
            sentence_encoder = BERTSemiSentenceEncoder(
                    pretrain_ckpt,
                    max_length,
    ...
    if opt.semi:
        ...
        # tokenizer = AutoTokenizer.from_pretrained('bert-base-cased', do_lower_case=True, output_hidden_states=True)
        relation_info_dict = load_relation_info_dict(tokenizer)
        if not os.path.exists('./data/train_wordrank.npy'):
            train_ori_examples = PBertDataset.preprocess('./data/train_wiki.json', relation_info_dict)
        ...
        if not os.path.exists('./data/val_wordrank.npy'):
            val_ori_examples = PBertDataset.preprocess('./data/val_wiki.json', relation_info_dict)
            val_adv_rank = GetAavRank(val_ori_examples, tokenizer)
        else:
            val_adv_rank_1 = np.load('./data/val_wordrank.npy', allow_pickle=True)
            val_adv_rank = val_adv_rank_1.tolist()
        ...
        with open('./data/' + str(opt.N) + '-' + str(opt.K) + '-na1.5-b4-new/train_adv_dataloader.pkl.gz', 'rb') as f:
            train_adv_dataloader = pickle.load(f)
        with open('./data/' + str(opt.N) + '-' + str(opt.K) + '-na1.5-b4-new/val_adv_dataloader.pkl.gz', 'rb') as f:
            val_adv_dataloader = pickle.load(f)
        # with open('./data/' + str(opt.N) + '-' + str(opt.K) + '-na' + str(opt.na_rate) + '-b4-new/train_adv_dataloader.pkl.gz', 'rb') as f:
        #     train_adv_dataloader = pickle.load(f)
        # with open('./data/' + str(opt.N) + '-' + str(opt.K) + '-na' + str(opt.na_rate) + '-b4-new/val_adv_dataloader.pkl.gz', 'rb') as f:
        #     val_adv_dataloader = pickle.load(f)
    ...
    if opt.adv:
        ...
        framework = FewShotREFramework(train_adv_dataloader, val_adv_dataloader, test_data_loader, semi=opt.semi, mrm=opt.mrm)
    else:
        framework = FewShotREFramework(train_data_loader, val_data_loader, test_data_loader)
    ...
    if model_name == 'proto':
        ...
        model = MRM(sentence_encoder, hidden_size=opt.hidden_size, dot=opt.dot, N=opt.N, K=opt.K, Q=opt.Q, na_rate=opt.na_rate, pns_rate=opt.pns_rate)
    else:
        raise NotImplementedError
    ...
    ckpt = 'checkpoint/{}-avg.pth.tar'.format(prefix)
    ...
    if not opt.only_test:
        ...
        framework.train(model, prefix, batch_size, trainN, N, K, Q,
                ...
                na_rate=opt.na_rate, val_step=opt.val_step, fp16=opt.fp16, pair=opt.pair, semi=opt.semi, mrm=opt.mrm,
                train_iter=opt.train_iter, val_iter=opt.val_iter, bert_optim=bert_optim,
                learning_rate=opt.lr, use_sgd_for_bert=opt.use_sgd_for_bert, grad_iter=opt.grad_iter, gpu=opt.gpu)
        ...
        if ckpt is None:
            ...
            ckpt = 'none'
    if not opt.codalab:
        acc = framework.eval(model, batch_size, N, K, Q, opt.test_iter, na_rate=opt.na_rate, ckpt=ckpt, pair=opt.pair, semi=opt.semi, mrm=opt.mrm, codalab=opt.codalab)
    else:
        acc = framework.eval(model, 1, N, K, Q, opt.test_iter, na_rate=opt.na_rate, ckpt=ckpt, pair=opt.pair, semi=opt.semi, mrm=opt.mrm, codalab=opt.codalab)
