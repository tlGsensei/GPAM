...
class FewShotREFramework:
    ...
    def __init__(self, train_data_loader, val_data_loader, test_data_loader, adv_data_loader=None, adv=False, d=None, semi=None, mrm=None):
        '''
        ...
        for it in range(start_iter, start_iter + train_iter):
            if pair:
                ...
                if torch.cuda.is_available():
                    ...
                    label = label.cuda()
                loss, pred = model(support, query, N_for_train, K, Q * N_for_train + na_rate * Q)
                pred = pred.cuda()
            ...
            if self.adv:
                sys.stdout.write('step: {0:4} | loss: {1:2.6f}, accuracy: {2:3.2f}%, dis_loss: {3:2.6f}, dis_acc: {4:2.6f}'.format(it + 1, iter_loss / iter_sample, 100 * iter_right / iter_sample, iter_loss_dis / iter_sample, 100 * iter_right_dis / iter_sample) + '\r')
            else:
                ...
                with open('logs/output.log', 'a') as f:
                    f.write('step: {0:4} | loss: {1:2.6f}, accuracy: {2:3.2f}%'.format(it + 1, iter_loss / iter_sample, 100 * iter_right / iter_sample) + '\n')
            sys.stdout.flush()

            # 每val_step步输出一次
            if (it + 1) % val_step == 0:
                acc = self.eval(model, B, N_for_eval, K, Q, val_iter, 
                        na_rate=na_rate, pair=pair, semi=semi)
                model.train()
                if acc > best_acc:
    ...
    def eval(self,
        ...
        if not codalab:
            ...
            iter_right = 0.0
            iter_sample = 0.0
            iter_right_close = 0.0
            iter_sample_close = 0.0
            iter_right_nota = 0.0
            iter_sample_nota = 0.0
            with torch.no_grad():
                for it in range(eval_iter):
                    if pair:
                        ...
                        if torch.cuda.is_available():
                            for k in batch:
                                batch[k] = batch[k].cuda()
                            label = label.cuda()
                        logits, pred = model(batch, N, K, Q * N + int(Q * na_rate))
                    elif semi:
                        ...
                        if torch.cuda.is_available():
                            ...
                            for k in query:
                                query[k] = query[k].cuda()
                            label = label.cuda()
                        logits, pred = model(support, query, N, K, Q * N + int(Q * na_rate))
                        pred = pred.cuda()
                    else:
                        support, query, label = next(eval_dataset)
                        if torch.cuda.is_available():
                    ...
                    # 这里将封闭集的类和NOTA类分开计算各自的accuracy
                    if na_rate == 0:
                        sys.stdout.write('[EVAL] step: {0:4} | total_accuracy: {1:3.2f}%'.format(
                                    it + 1, 100 * iter_right / iter_sample) + '\r')
                    else:
                        sys.stdout.write('[EVAL] step: {0:4} | total_accuracy: {1:3.2f}% | close_accuracy: {2:3.2f}% | NOTA_accuracy: {3:3.2f}%'.format(
                                    it + 1, 100 * iter_right / iter_sample, 100 * iter_right_close / iter_sample_close, 100 * iter_right_nota / iter_sample_nota) + '\r')
        ...
        # 在codalab上的测试结果
            ...
            if na_rate == 0:
                eval_dataset = json.load(open('./test/test_wiki_input-{}-{}.json'.format(N, K)))
            else:
                eval_dataset = json.load(open('./test/test_wiki_input-{}-{}-0.5.json'.format(N, K)))
            eval_iter = len(eval_dataset)
            pred_set = []
            print("Use codalab test dataset")
            if ckpt != 'none':
                state_dict = self.__load_model__(ckpt)['state_dict']
                own_state = model.state_dict()
                for name, param in state_dict.items():
            ...
            with torch.no_grad():
                for it in range(eval_iter):
                    ...
                    print(pred)
                    pred_set.append(pred.item())

                json_data = json.dumps(pred_set)
                with open('pred_mrm/pred-{}-{}.json'.format(N, K), 'w') as file:
