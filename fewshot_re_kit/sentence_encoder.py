...
class BERTSemiSentenceEncoder(nn.Module):
    ...
    def forward(self, inputs):
        tensor_range = torch.arange(inputs['word'].size()[0])
        # outputs[0]的形状是(N * batch_size, sequence_length, hidden_size)

        outputs1 = self.bert((inputs['word'][:, 0, :]), attention_mask=inputs['word'][:, 2, :])
        h_state1 = outputs1[0][tensor_range, inputs["pos1"]]
        t_state1 = outputs1[0][tensor_range, inputs["pos2"]]
        state1 = torch.cat((h_state1, t_state1), -1)

        outputs2 = self.bert((inputs['head_debiased'][:, 0, :]), attention_mask=inputs['head_debiased'][:, 2, :])
        h_state2 = outputs2[0][tensor_range, inputs["pos1"]]
        t_state2 = outputs2[0][tensor_range, inputs["pos2"]]
        state2 = torch.cat((h_state2, t_state2), -1)

        outputs3 = self.bert((inputs['tail_debiased'][:, 0, :]), attention_mask=inputs['tail_debiased'][:, 2, :])
        h_state3 = outputs3[0][tensor_range, inputs["pos1"]]
        t_state3 = outputs3[0][tensor_range, inputs["pos2"]]
        state3 = torch.cat((h_state3, t_state3), -1)

        outputs4 = self.bert((inputs['context_debiased'][:, 0, :]), attention_mask=inputs['context_debiased'][:, 2, :])
        h_state4 = outputs4[0][tensor_range, inputs["pos1"]]
        t_state4 = outputs4[0][tensor_range, inputs["pos2"]]
        state4 = torch.cat((h_state4, t_state4), -1)
